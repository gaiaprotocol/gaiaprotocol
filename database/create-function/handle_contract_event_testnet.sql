...
AS $$BEGIN
  ...
  IF NEW.contract_address = '0x9322C4A5E5725262C9960aDE87259d1cE2812412' THEN
    IF NEW.event_name = 'ClanCreated' THEN
      INSERT INTO clans (chain, id, owner, name, logo_image_url, logo_thumbnail_url, description, created_at)
      SELECT NEW.chain, NEW.event_arguments[1]::bigint, NEW.event_arguments[0], pc.name, pc.logo_image_url, pc.logo_thumbnail_url, pc.description, NOW()
  ...
  IF NEW.contract_address = '0xc78c189C24379857A80635624877E02306de3EE1' THEN
    IF NEW.event_name = 'MaterialCreated' THEN
      INSERT INTO materials (chain, address, owner, name, symbol, logo_image_url, logo_thumbnail_url, description, created_at)
      SELECT NEW.chain, NEW.contract_address, NEW.event_arguments[0], pm.name, pm.symbol, pm.logo_image_url, pm.logo_thumbnail_url, pm.description, NOW()
